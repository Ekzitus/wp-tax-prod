...
function create_product_post_type() {
    $labels = array(
        ...
        'parent_item_colon'  => 'Родительские товары:',
...
function create_catalog_taxonomy() {
    $labels = array(
        ...
        'new_item_name'     => 'Новое имя каталога',
        'menu_name'         => 'Каталоги',
    );
...
function create_attributes_taxonomy() {
    $labels = array(
        ...
        'new_item_name'     => 'Новое имя атрибута',
        'menu_name'         => 'Атрибуты',
    );
    ...
    register_taxonomy('attributes', 'product', $args);
}
add_action('init', 'create_attributes_taxonomy');
function custom_rewrite_rules_for_attributes() {
    add_rewrite_rule(
        '^catalog/([^/]+)/([^/]+(/[^/]+)*)?$',
        'index.php?taxonomy=catalog&term=$matches[1]&attributes=$matches[2]',
        'top'
    );
}
add_action('init', 'custom_rewrite_rules_for_attributes');
